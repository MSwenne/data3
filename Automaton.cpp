...
std::set<unsigned> Automaton::getAlphabet(){
	return alphabet;
}
...
void Automaton::intersect(Automaton& fa1, Automaton& fa2){
	...
	while(!remain.empty()){
		...
		addState(state);
		remain.pop();
		if(fa1.finalStates.find(pairs.first)!=fa1.finalStates.end() || fa2.finalStates.find(pairs.second)!=fa2.finalStates.end()){
		...
		for(it = bitSet.begin(); it != bitSet.end(); ++it){
			...
			if(transf1.find(*it) != transf1.end() && transf2.find(*it) != transf2.end()){
...
void Automaton::complement(Automaton& fa){
	...
	std::set<State> allstates = fa.states;
	std::set<State>::iterator it;
	for(it = allstates.begin(); it != allstates.end(); ++it){
		if(fa.finalStates.find(*it) == fa.finalStates.end()){
			markFinal(*it);
		}
	}
}
...
void Automaton::makeDeterministic(Automaton& fa){
	std::cout << "size  " << fa.transitions.size()  << std::endl;

	if(hasEpsilon(fa)){
		std::cout << "eliminateLambda " << std::endl;
		eliminateLambda(fa); 
	}
	else{
...
bool Automaton::hasEpsilon(Automaton& fa){
	std::map<State, std::map<BitVector, std::set<State> > >::iterator it;
	it = fa.transitions.begin();
	std::map<BitVector, std::set<State> > bitMap = it->second;
	std::map<BitVector, std::set<State> >::iterator bitIt;
	bitIt = bitMap.begin();
	BitVector vec = bitIt->first;
